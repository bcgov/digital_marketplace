...
const readMany: crud.ReadMany<Session, db.Connection> = (
  ...
  >(async (request) => {
    ...
    if (
      !(await permissions.readManyTWUResourceQuestionResponseEvaluationsForConsensus(
        connection,
        request.session,
    ...
    const dbResult =
      await db.readManyTWUResourceQuestionResponseEvaluationsForConsensus(
        connection,
        request.session,
        request.params.proposalId
      );
...
const readOne: crud.ReadOne<Session, db.Connection> = (
  ...
  >(async (request) => {
    ...
    if (
      !(await permissions.readOneTWUResourceQuestionResponseEvaluation(
        connection,
        request.session,
...
> = (connection: db.Connection) => {
  return {
    ...
    async validateRequestBody(request) {
      ...
      if (
        !(await permissions.createTWUResourceQuestionResponseEvaluation(
          connection,
          request.session,
          validatedTWUProposal.value
        ))
