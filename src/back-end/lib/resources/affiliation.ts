...
const resource: Resource = {
  ...
  readMany(connection) {
    return nullRequestBodyHandler<JsonResponseBody<AffiliationSlim[] | AffiliationMember[] | string[]>, Session>(async request => {
  ...
  create(connection) {
    return {
      ...
      async validateRequestBody(request) {
        ...
        const validatedUser = await db.readOneUserByEmail(connection, validatedUserEmail.value, false, true);
