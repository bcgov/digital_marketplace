...
const resource: Resource = {
  ...
  readMany(connection) {
    return nullRequestBodyHandler<
      JsonResponseBody<ReadManyResponseBody | ReadManyResponseValidationErrors>,
      Session
    >(async (request) => {
      const respond = (
        code: number,
        body: ReadManyResponseBody | ReadManyResponseValidationErrors
      ...
      const dbResult = await db.readManyOrganizations(
        connection,
        request.session,
        false,
        validatedPageIndex.value,
  ...
  readOne(connection) {
    return nullRequestBodyHandler<
      JsonResponseBody<Organization | string[]>,
      Session
    >(async (request) => {
      const respond = (code: number, body: Organization | string[]) =>
        basicResponse(code, request.session, makeJsonResponseBody(body));
      ...
      if (
        await permissions.readOneOrganization(
          connection,
          request.session,
          validatedId.value
        )
  ...
  create(connection) {
    return {
      ...
      async validateRequestBody(request) {
        ...
        const validatedLogoImageFile = await optionalAsync(logoImageFile, (v) =>
          validateFileRecord(connection, v)
        );
        const validatedWebsiteUrl =
          orgValidation.validateWebsiteUrl(websiteUrl);
        const validatedStreetAddress1 =
          orgValidation.validateStreetAddress1(streetAddress1);
        const validatedStreetAddress2 =
          orgValidation.validateStreetAddress2(streetAddress2);
        ...
        const validatedContactName =
          orgValidation.validateContactName(contactName);
        const validatedContactTitle =
          orgValidation.validateContactTitle(contactTitle);
        const validatedContactEmail =
          orgValidation.validateContactEmail(contactEmail);
        const validatedContactPhone =
          orgValidation.validateContactPhone(contactPhone);
        ...
        if (
          ...
          if (
            !(await permissions.createOrganization(
              connection,
              request.session
            )) ||
          ...
          return valid({
            ...
            body: {
              ...
              logoImageFile:
                isValid(validatedLogoImageFile) &&
                validatedLogoImageFile.value &&
                validatedLogoImageFile.value.id,
        ...
        valid: async (request) => {
          if (!request.session) {
            return basicResponse(
              401,
              request.session,
              makeJsonResponseBody({ permissions: [permissions.ERROR_MESSAGE] })
            );
          }
          const dbResult = await db.createOrganization(
            connection,
            request.body.session.user.id,
            request.body.body,
            request.session
          );
          if (isInvalid(dbResult)) {
  ...
  update(connection) {
    return {
      ...
      async validateRequestBody(
        ...
        Validation<ValidatedUpdateRequestBody, UpdateValidationErrors>
      > {
        if (!request.body) {
          return invalid({ organization: adt("parseFailure" as const) });
        }
        if (
          !(await permissions.updateOrganization(
            connection,
            request.session,
            request.params.id
          )) ||
        ...
        switch (request.body.tag) {
          case "updateProfile": {
            ...
            const validatedLegalName =
              orgValidation.validateLegalName(legalName);
            const validatedLogoImageFile = await optionalAsync(
              logoImageFile,
            ...
            if (
              ...
              return invalid({
                organization: adt("updateProfile" as const, {
                  ...
                  legalName: getInvalidValue(validatedLegalName, undefined),
                  logoImageFile: getInvalidValue(
                    validatedLogoImageFile,
                    undefined
                  ),
                  websiteUrl: getInvalidValue(validatedWebsiteUrl, undefined),
                  contactName: getInvalidValue(validatedContactName, undefined),
                  contactTitle: getInvalidValue(
                    validatedContactTitle,
                    undefined
                  ),
                  contactEmail: getInvalidValue(
                    validatedContactEmail,
                    undefined
                  ),
                  contactPhone: getInvalidValue(
                    validatedContactPhone,
                    undefined
                  ),
                  streetAddress1: getInvalidValue(
                    validatedStreetAddress1,
                    undefined
                  ),
                  streetAddress2: getInvalidValue(
                    validatedStreetAddress2,
                    undefined
                  ),
                  city: getInvalidValue(validatedCity, undefined),
  ...
  delete(connection) {
    return {
      async validateRequestBody(
        request
      ): Promise<Validation<DeleteValidatedReqBody, DeleteValidationErrors>> {
        if (
          !(await permissions.deleteOrganization(
            connection,
            request.session,
            request.params.id
          )) ||
      ...
      respond: wrapRespond({
        valid: async (request) => {
          ...
          const dbResult = await db.updateOrganization(
            connection,
            {
          ...
          if (
            dbResult.value.owner &&
            dbResult.value.owner.id !== request.body.session.user.id
          ) {
            orgNotifications.handleOrganizationArchived(
              connection,
              dbResult.value
            );
          }
