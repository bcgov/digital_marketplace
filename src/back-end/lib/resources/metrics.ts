import { TOTAL_AWARDED_COUNT_OFFSET, TOTAL_AWARDED_VALUE_OFFSET } from 'back-end/config';
...
const resource: Resource = {
  ...
  readMany(connection) {
    return nullRequestBodyHandler<JsonResponseBody<OpportunityMetrics[] | string[]>, Session>(async request => {
      const respond = (code: number, body: OpportunityMetrics[] | string[]) => basicResponse(code, request.session, makeJsonResponseBody(body));
