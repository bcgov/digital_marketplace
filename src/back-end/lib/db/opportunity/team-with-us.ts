...
/**
 * Checks if all proposals for a TWU opportunity are in EVALUATED_CHALLENGE state.
 * If yes, changes opportunity to PROCESSING state.
 * If any proposal is not evaluated in challenge phase and opportunity is in PROCESSING state,
 * changes it back to EVALUATION_CHALLENGE state.
 */
export const updateTWUOpportunityProcessingStatus = tryDb<
...
>(async (connection, opportunityId, session) => {
  ...
  // We only care about proposals that made it to the challenge phase
  const proposals = await connection<{ status: TWUProposalStatus }>(
    "twuProposals"
  )
    .join("twuProposalStatuses", function () {
      this.on(
        "twuProposals.id",
        "=",
        "twuProposalStatuses.proposal"
      ).andOnNotNull("twuProposalStatuses.status");
    })
    .where({
      "twuProposals.opportunity": opportunityId
    })
    .whereNotIn("twuProposalStatuses.status", [
    ...
    .select("twuProposalStatuses.status")
    .orderBy("twuProposalStatuses.createdAt", "desc")
    .groupBy(
      "twuProposals.id",
      "twuProposalStatuses.status",
      "twuProposalStatuses.createdAt"
    );

  // Get current opportunity status
  const currentOpportunity = await connection<{ status: TWUOpportunityStatus }>(
    "twuOpportunityStatuses"
  )
    .where("opportunity", opportunityId)
    .select("status")
  ...
  if (
    ...
    newStatus = TWUOpportunityStatus.Processing;
    note =
  ...
  if (newStatus !== currentStatus) {
    return await updateTWUOpportunityStatus(
      connection,
      opportunityId,
      newStatus,
      note,
