...
export const readOneUserByEmail = tryDb<[string, boolean?, boolean?], User | null>(async (connection, email, allowInactive = false, vendorOnly = false) => {
  const where = {
    email,
    status: (allowInactive ? '*' : UserStatus.Active),
    type: (vendorOnly ? UserType.Vendor : '*')
  };
