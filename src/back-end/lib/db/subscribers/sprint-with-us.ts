...
type DeleteSWUOpportunitySubscriberParams =
  CreateSWUOpportunitySubscriberParams;
...
async function rawSWUOpportunitySubscriberToSWUOpportunitySubscriber(
  connection: Connection,
  session: Session,
  raw: RawSWUOpportunitySubscriber
): Promise<SWUOpportunitySubscriber> {
...
export const readOneSWUSubscriberByOpportunityAndUser = tryDb<
  [Id, Id, Session],
  SWUOpportunitySubscriber | null
>(async (connection, opportunityId, userId, session) => {
  const result = await connection<RawSWUOpportunitySubscriber>(
  ...
  return valid(
    result
      ? await rawSWUOpportunitySubscriberToSWUOpportunitySubscriber(
          connection,
          session,
          result
        )
...
export const readManySWUSubscribedUsers = tryDb<[Id], User[]>(
  async (connection, opportunity) => {
    const results = await connection<RawUser>("users")
      .join(
        "swuOpportunitySubscribers as subscribers",
        "users.id",
        "=",
        "subscribers.user"
      )
...
export const createSWUOpportunitySubscriber = tryDb<
  [CreateSWUOpportunitySubscriberParams, Session],
...
export const deleteSWUOpportunitySubscriber = tryDb<
  [DeleteSWUOpportunitySubscriberParams, Session],
  SWUOpportunitySubscriber
>(async (connection, subscriber, session) => {
  const [result] = await connection<RawSWUOpportunitySubscriber>(
    "swuOpportunitySubscribers"
