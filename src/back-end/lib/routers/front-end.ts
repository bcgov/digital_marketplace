    ...
    handler: nullRequestBodyHandler(async request => {
      const fileResponseBody = (() => {
        ...
        if (isSupportedBrowser || filePath.match(ALWAYS_ALLOWED_FILES)) {
          ...
          return tryMakeAnyCompressedFileResponseBody(filePath, acceptEncodingHeader)
              || tryMakeAnyCompressedFileResponseBody(fallbackFilePath, acceptEncodingHeader);
        } else {
          // Otherwise, indicate that browser is unsupported.
          return tryMakeAnyCompressedFileResponseBody(join(FRONT_END_BUILD_DIR, 'unsupported-browser.html'), acceptEncodingHeader);
        }
      })();
      return {
        code: 200,
