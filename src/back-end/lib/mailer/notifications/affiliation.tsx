...
export async function handleUserInvited(
...
export async function handleUserAcceptedInvitation(
  connection: db.Connection,
  affiliation: Affiliation
): Promise<void> {
  // Notify owner of the affiliated organization
  const owner = getValidValue(
    await db.readOneOrganizationOwner(connection, affiliation.organization.id),
    null
  );
  if (owner) {
...
export async function handleUserRejectedInvitation(
  connection: db.Connection,
  affiliation: Affiliation
): Promise<void> {
  // Notify owner of the affiliated organization
...
export async function handleMemberLeavesTeam(
  connection: db.Connection,
  affiliation: Affiliation
): Promise<void> {
  // Notify owner of the affiliated organization
  ...
  return [
    {
      ...
      html: templates.simple({
        ...
        body: (
          <div>
            <p>
              {organization.legalName} has requested that you join their team on
            ...
            <p>
              You can approve or reject your membership with{" "}
              {organization.legalName} by using one of the buttons below.
            </p>
            <p>
        ...
        callsToAction: [
          approveJoinRequestCallToAction(recipient, affiliation),
          rejectJoinRequestCallToAction(recipient, affiliation)
        ]
      })
    }
  ];
...
export async function approvedRequestToJoinT(
  recipient: User,
  affiliation: Affiliation
): Promise<Emails> {
  ...
  return [
    {
      ...
      html: templates.simple({
        ...
        body: (
          <div>
            <p>
              {memberName} has approved your request to join {organizationName}
              {"'"}s team on the Digital Marketplace.
            </p>
            <p>
...
export async function rejectRequestToJoinT(
  recipient: User,
  affiliation: Affiliation
): Promise<Emails> {
  const memberName = affiliation.user.name;
  ...
  return [
    {
      ...
      html: templates.simple({
        ...
        body: (
          <div>
            <p>
              {memberName} has rejected your request to join {organizationName}
              {"'"}s team on the Digital Marketplace.
            </p>
          </div>
        )
...
export async function membershipCompleteT(
  affiliation: Affiliation
): Promise<Emails> {
  const recipient = affiliation.user;
  const organizationName = affiliation.organization.legalName;
...
export async function memberLeavesT(
  recipient: User,
  affiliation: Affiliation
): Promise<Emails> {
  const organizationName = affiliation.organization.legalName;
...
export function viewOrganizationCallToAction(
...
export function viewMyOrganizationsCallToAction(
  user: User
): templates.LinkProps {
  return {
...
export function approveJoinRequestCallToAction(
  user: User,
  affiliation: Affiliation
) {
  return {
    ...
    url: templates.makeUrl(
      `users/${user.id}?tab=organizations&invitationAffiliationId=${affiliation.id}&invitationResponse=approve`
    ),
    style: templates.styles.classes.buttonSuccess
  };
}

export function rejectJoinRequestCallToAction(
  user: User,
  affiliation: Affiliation
) {
  return {
    text: "Reject",
    url: templates.makeUrl(
      `users/${user.id}?tab=organization&invitationAffiliationId=${affiliation.id}&invitationResponse=reject`
    ),
