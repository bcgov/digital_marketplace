...
const generateSQLAlterTableEnums = (
  tableName: string,
  columnName: string,
  enums: string[]
): string => {
...
export async function up(connection: Knex): Promise<void> {
  ...
  await connection.schema.raw(
    generateSQLAlterTableEnums(
      "cwuOpportunityStatuses",
      "event",
      Object.values(CWUOpportunityEvent)
    )
  );
  ...
  await connection.schema.raw(
    generateSQLAlterTableEnums(
      ...
      "event",
      Object.values(SWUOpportunityEvent)
    )
  );
  ...
  await connection.schema.createTable(
    ...
    (table) => {
      table
        .uuid("event")
        .references("id")
        .inTable("cwuOpportunityStatuses")
        .notNullable();
  ...
  await connection.schema.createTable(
    ...
    (table) => {
      table
        .uuid("event")
        .references("id")
        .inTable("swuOpportunityStatuses")
        .notNullable();
      table.uuid("file").references("id").inTable("files").notNullable();
      table.primary(["event", "file"]);
    }
...
export async function down(connection: Knex): Promise<void> {
  ...
  await connection.schema.raw(
    generateSQLAlterTableEnums(
      "cwuOpportunityStatuses",
      "event",
      Object.values(OldCWUOpportunityEvent)
    )
  );
  ...
  await connection.schema.raw(
    generateSQLAlterTableEnums(
      "swuOpportunityStatuses",
      "event",
      Object.values(OldSWUOpportunityEvent)
    )
  );
