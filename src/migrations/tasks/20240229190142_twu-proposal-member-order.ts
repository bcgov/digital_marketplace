...
export async function up(connection: Knex): Promise<void> {
  ...
  logger.info("Added order column to twuProposalMember table");

  /**
   * Fetch order explicitly from the resource
   */
  await connection("twuProposalMember as tpm").update({
    order: connection("twuResources as tr")
      .select("order")
      .where("id", connection.raw("??", ["tpm.resource"]))
  });
...
export async function down(connection: Knex): Promise<void> {
  ...
  logger.info("Dropped order column in the twuProposalMember table");
}
