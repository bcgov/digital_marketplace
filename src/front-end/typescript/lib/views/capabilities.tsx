...
const Capability: View<CapabilityProps> = ({ capability, fullTime, checked, index, showChecked, showFullOrPartTime }) => {
  return (
    <div className='border-right border-bottom d-flex flex-nowrap align-items-center p-2'>
      ...
      {showFullOrPartTime
        ? (<div style={{ width: '2rem', height: '1.75rem' }} className={`d-flex justify-content-center align-items-center flex-shrink-0 small ml-auto rounded font-weight-bold text-white ${fullTime ? 'bg-c-capability-ft-bg' : 'bg-c-capability-pt-bg'}`}>{fullTime ? 'F/T' : 'P/T'}</div>)
        : null}
    </div>
  );
};
