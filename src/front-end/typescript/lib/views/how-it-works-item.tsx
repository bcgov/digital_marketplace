  ...
  return (
    <Row className={className}>
      <Col xs='12' className='d-flex flex-nowrap align-items-start'>
        <div className={`d-none d-sm-flex justify-content-center align-items-center rounded-circle bg-${bgColor} p-3 mr-4`}>
          {symbol_.tag === 'icon'
            ? (<Icon name={symbol_.value} color={fgColor} width={2} height={2} />)
            : (<div className={`d-flex justify-content-center align-items-center flex-nowrap h2 mb-0 text-${fgColor} position-relative`} style={{ width: '2rem', height: '2rem' }}>
                {symbol_.value}
              </div>)}
        </div>
        <div className='d-flex flex-column flex-grow-1'>
          <div className='d-flex flex-nowrap align-items-center mb-2'>
            <div className={`d-block d-sm-none text-${bgColor} mr-2`}>
