    ...
    <div
      ...
      {!isInTable && (
        <Gutter>
          <div
            className={cn(
              "tw:slate-blockToolbarWrapper",
              "tw:flex tw:h-[1.5em]",
              isInColumn && "tw:h-4"
            )}>
            <div
              className={cn(
                "tw:slate-blockToolbar tw:relative tw:w-4.5",
                "tw:pointer-events-auto tw:mr-1 tw:flex tw:items-center",
                isInColumn && "tw:mr-1.5"
              )}>
              <Button
                ref={handleRef}
                variant="ghost"
                className="tw:absolute tw:-left-0 tw:h-6 tw:w-full tw:p-0"
      ...
      <div
        ref={previewRef}
        className={cn("tw:absolute tw:-left-0 tw:hidden tw:w-full")}
      ...
      <div
        ref={nodeRef}
        className="tw:slate-blockWrapper tw:flow-root"
        onContextMenu={(event) =>
          editor
...
function Gutter({
  ...
  return (
    <div
      ...
      className={cn(
        "tw:slate-gutterLeft",
        "tw:absolute tw:top-0 tw:z-50 tw:flex tw:h-full tw:-translate-x-full tw:cursor-text tw:hover:opacity-100 tw:sm:opacity-0",
        getPluginByType(editor, element.type)?.node.isContainer
          ? "tw:group-hover/container:opacity-100"
          : "tw:group-hover:opacity-100",
        isSelectionAreaVisible && "tw:hidden",
...
const DropLine = React.memo(function DropLine({
  ...
  return (
    <div
      ...
      className={cn(
        "tw:slate-dropLine",
        "tw:absolute tw:inset-x-0 tw:h-0.5 tw:opacity-100 tw:transition-opacity",
        "tw:bg-brand/50",
        dropLine === "top" && "tw:-top-px",
        dropLine === "bottom" && "tw:-bottom-px",
        className
      )}
    />
  );
