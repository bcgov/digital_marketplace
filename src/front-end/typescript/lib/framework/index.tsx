...
export function immutable<State extends object>(
  state: State
): Immutable<State> {
...
export type UpdateReturnValue<State extends object, Msg> = [
  Immutable<State>,
  ((
...
export type Update<State extends object, Msg> = (
...
export type ViewElementChildren<Props = any> =
  | ViewElement<Props>
  | string
...
export type ComponentView<
  ...
  Msg,
  ReturnValue = ViewElement
> = View<ComponentViewProps<State, Msg>, ReturnValue>;
...
export function mapComponentDispatch<ParentMsg, ChildMsg>(
...
export function updateComponentChild<
  PS extends object,
  PM,
  CS extends object,
  CM
>(params: UpdateChildParams<PS, PM, CS, CM>): UpdateReturnValue<PS, PM> {
  ...
  if (newAsyncChildState || newAsyncUpdateAfterState) {
    asyncStateUpdate = async (
      state: Immutable<PS> | any,
      dispatch: Dispatch<PM>
    ) => {
...
export function isGlobalMsg<Msg, Route>(
  msg: GlobalComponentMsg<Msg, Route>
): msg is GlobalMsg<Route> {
  const globalMsg = msg as GlobalMsg<Route>;
  return (
    globalMsg.tag === "@toast" ||
    globalMsg.tag === "@newRoute" ||
    globalMsg.tag === "@replaceRoute" ||
    globalMsg.tag === "@newUrl" ||
    globalMsg.tag === "@replaceUrl" ||
    globalMsg.tag === "@reload"
  );
}

export function mapGlobalComponentMsg<MsgA, MsgB, Route>(
  msg: GlobalComponentMsg<MsgA, Route>,
  map: (msg: GlobalComponentMsg<MsgA, Route>) => GlobalComponentMsg<MsgB, Route>
): GlobalComponentMsg<MsgB, Route> {
  return isGlobalMsg(msg) ? msg : map(msg);
}

export function mapGlobalComponentDispatch<ParentMsg, ChildMsg, Route>(
  dispatch: Dispatch<GlobalComponentMsg<ParentMsg, Route>>,
  fn: (
    childMsg: GlobalComponentMsg<ChildMsg, Route>
  ) => GlobalComponentMsg<ParentMsg, Route>
): Dispatch<GlobalComponentMsg<ChildMsg, Route>> {
  return (childMsg) => {
...
export function updateGlobalComponentChild<
  PS extends object,
  PM,
  CS extends object,
  CM,
...
>(
  params: UpdateChildParams<
    PS,
    GlobalComponentMsg<PM, Route>,
    CS,
    GlobalComponentMsg<CM, Route>
  >
): UpdateReturnValue<PS, GlobalComponentMsg<PM, Route>> {
  const { childStatePath, childUpdate, childMsg, mapChildMsg, updateAfter } =
    params;
  let { state } = params;
  const childState = state.getIn(childStatePath);
  ...
  state = state.setIn(childStatePath, newChildState);
  let newAsyncUpdateAfterState: UpdateReturnValue<
    PS,
    GlobalComponentMsg<PM, Route>
  >[1];
  if (updateAfter) {
  ...
  if (newAsyncChildState) {
    asyncStateUpdate = async (
      state: Immutable<PS> | any,
      dispatch: Dispatch<GlobalComponentMsg<PM, Route>>
    ) => {
...
export type PageInit<RouteParams, SharedState, State, Msg> = Init<
  PageInitParams<RouteParams, SharedState, Msg>,
  State
>;
...
export type PageGetMetadata<State extends object> = (
  state: Immutable<State>
) => PageMetadata;
...
export type PageGetAlerts<State extends object, Msg> = (
  state: Immutable<State>
) => PageAlerts<Msg>;
...
export function mapPageAlerts<MsgA, MsgB, Route>(
  alerts: PageAlerts<GlobalComponentMsg<MsgA, Route>>,
  mapMsg: (
    msgA: GlobalComponentMsg<MsgA, Route>
  ) => GlobalComponentMsg<MsgB, Route>
): PageAlerts<GlobalComponentMsg<MsgB, Route>> {
  const { info, warnings, errors } = alerts;
  return {
    info: info?.map((i) => ({
      ...i,
      dismissMsg: i.dismissMsg && mapGlobalComponentMsg(i.dismissMsg, mapMsg)
    })),
...
export function mergePageAlerts<Msg>(
  a: PageAlerts<Msg>,
  b: PageAlerts<Msg>
): PageAlerts<Msg> {
  return {
...
export type PageGetBreadcrumbs<State extends object, Msg> = (
  state: Immutable<State>
) => PageBreadcrumbs<Msg>;
...
export function mapPageBreadcrumbsMsg<MsgA, MsgB, Route>(
  breadcrumbs: PageBreadcrumbs<GlobalComponentMsg<MsgA, Route>>,
  mapMsg: (
    msgA: GlobalComponentMsg<MsgA, Route>
  ) => GlobalComponentMsg<MsgB, Route>
): PageBreadcrumbs<GlobalComponentMsg<MsgB, Route>> {
...
export function mapPageModalGlobalComponentMsg<MsgA, MsgB, Route>(
  modal: PageModal<GlobalComponentMsg<MsgA, Route>> | null,
  mapMsg: (
    msgA: GlobalComponentMsg<MsgA, Route>
  ) => GlobalComponentMsg<MsgB, Route>
): PageModal<GlobalComponentMsg<MsgB, Route>> | null {
  return mapPageModalMsg(modal, (msg) => mapGlobalComponentMsg(msg, mapMsg));
}

export function mapPageModalMsg<MsgA, MsgB>(
  modal: PageModal<MsgA> | null,
  mapMsg: (msgA: MsgA) => MsgB
): PageModal<MsgB> | null {
  if (!modal) {
    return null;
  }
...
export type PageGetModal<State extends object, Msg> = (
  state: Immutable<State>
) => PageModal<Msg> | null;
...
export type PageGetContextualActions<State extends object, Msg> = (
  props: ComponentViewProps<State, Msg>
) => PageContextualActions | null;
...
export type AppMsg<Msg, Route> =
  | GlobalComponentMsg<Msg, Route>
  | Router.IncomingRouteMsg<Route>;

export type AppGetAlerts<State extends object, Msg> = (
  props: ComponentViewProps<State, Msg>
) => PageAlerts<Msg>;

export interface AppComponent<State extends object, Msg, Route>
  extends Component<null, State, AppMsg<Msg, Route>> {
  router: Router.Router<Route>;
}

export function mapAppDispatch<ParentMsg, ChildMsg, Route>(
  dispatch: Dispatch<AppMsg<ParentMsg, Route>>,
  fn: (
    childMsg: GlobalComponentMsg<ChildMsg, Route>
  ) => AppMsg<ParentMsg, Route>
): Dispatch<GlobalComponentMsg<ChildMsg, Route>> {
  return (childMsg) => {
    const mappedMsg = mapGlobalComponentMsg(
      childMsg,
      (msg) => fn(msg) as GlobalComponentMsg<ParentMsg, Route>
    );
    return dispatch(mappedMsg);
  };
}

export interface InitAppChildPageParams<
  ParentState extends object,
  ParentMsg,
  ChildRouteParams,
  ChildState extends object,
...
export async function initAppChildPage<
  ParentState extends object,
  ParentMsg,
  ChildRouteParams,
  ChildState extends object,
  ...
  Route
>(
  params: InitAppChildPageParams<
    ParentState,
    AppMsg<ParentMsg, Route>,
    ChildRouteParams,
    ChildState,
    GlobalComponentMsg<ChildMsg, Route>,
    SharedState
  >
): Promise<Immutable<ParentState>> {
...
export function updateAppChild<
  PS extends object,
  PM,
  CS extends object,
  CM,
...
>(
  params: UpdateChildParams<
    ...
    GlobalComponentMsg<CM, Route>
  >
): UpdateReturnValue<PS, AppMsg<PM, Route>> {
  const { childStatePath, childUpdate, childMsg, mapChildMsg, updateAfter } =
    params;
  let { state } = params;
  const childState = state.getIn(childStatePath);
  ...
  if (newAsyncChildState) {
    asyncStateUpdate = async (
      state: Immutable<PS> | any,
      dispatch: Dispatch<AppMsg<PM, Route>>
    ) => {
...
export interface UpdateChildPageParams<
  PS extends object,
  PM,
  CS extends object,
  CM
> extends UpdateChildParams<PS, PM, CS, CM> {
...
export function updateAppChildPage<
  PS extends object,
  PM,
  CS extends object,
  CM,
...
>(
  ...
  const asyncStateUpdate = async (
    ...
    if (newAsyncState) {
      newState = await newAsyncState(state, dispatch);
    }
    if (!newState) {
      return state;
    }
  ...
  return [newState, asyncStateUpdate];
}

// State Manager.

export type StateSubscription<State extends object, Msg> = (
  state: Immutable<State>,
  dispatch: Dispatch<Msg>
) => void;

export type StateSubscribe<State extends object, Msg> = (
  fn: StateSubscription<State, Msg>
) => boolean;

export type StateUnsubscribe<State extends object, Msg> = (
  fn: StateSubscription<State, Msg>
) => boolean;
...
export async function start<
  State extends object,
  Msg extends ADT<any, any>,
  Route
>(
  app: AppComponent<State, Msg, Route>,
  element: HTMLElement,
  debug: boolean
): Promise<StateManager<State, AppMsg<Msg, Route>>> {
  // Initialize state.
  ...
  const stateSubscriptions: Array<
    StateSubscription<State, AppMsg<Msg, Route>>
  > = [];
  const msgSubscriptions: Array<MsgSubscription<AppMsg<Msg, Route>>> = [];
  const stateSubscribe: StateSubscribe<State, AppMsg<Msg, Route>> = (fn) =>
    (stateSubscriptions.push(fn) && true) || false;
  const stateUnsubscribe: StateUnsubscribe<State, AppMsg<Msg, Route>> = (fn) =>
    (remove(stateSubscriptions, (a) => a === fn) && true) || false;
  const msgSubscribe: MsgSubscribe<AppMsg<Msg, Route>> = (fn) =>
    (msgSubscriptions.push(fn) && true) || false;
  const msgUnsubscribe: MsgUnsubscribe<AppMsg<Msg, Route>> = (fn) =>
    (remove(msgSubscriptions, (a) => a === fn) && true) || false;
  ...
  const routeManager = Router.makeRouteManager<State, Msg, Route>(
    app.router,
    dispatch
  );
