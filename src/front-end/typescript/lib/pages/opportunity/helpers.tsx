...
export const isNoteValid = (state, MAX_NOTE_LENGTH) => {
  return isNoteMinLengthValid(state) &&
    isNoteMaxLengthValid(state, MAX_NOTE_LENGTH)
    ? true
    : false;
};
...
const isAttachmentFormatValid = (state, SUPPORTED_NOTE_ATTACHMENT_FORMATS) => {
  if (state.attachments.newAttachments.length >= 1) {
    return (
      state.attachments.newAttachments.filter(
        (attachment) =>
          !SUPPORTED_NOTE_ATTACHMENT_FORMATS.includes(attachment.file.type)
      ).length === 0
    );
  }
  return true;
...
export const isAttachmentValid = (
  ...
  SUPPORTED_NOTE_ATTACHMENT_FORMATS
) => {
  return isAttachmentFormatValid(state, SUPPORTED_NOTE_ATTACHMENT_FORMATS) &&
    isAttachmentSizeValid(state, MAX_NOTE_ATTACHMENT_SIZE)
    ? true
    : false;
};

export const createAttachmentError = (
  state,
  MAX_NOTE_ATTACHMENT_SIZE,
  SUPPORTED_NOTE_ATTACHMENT_FORMATS
) => {
  ...
  if (
    !isAttachmentFormatValid(state, SUPPORTED_NOTE_ATTACHMENT_FORMATS) &&
    !isAttachmentSizeValid(state, MAX_NOTE_ATTACHMENT_SIZE)
  ) {
    return (
      <>
  ...
  if (!isAttachmentSizeValid(state, MAX_NOTE_ATTACHMENT_SIZE)) {
    return <Alert color="danger">{sizeError}</Alert>;
  }
  return null;
};
