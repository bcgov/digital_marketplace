...
export type InnerMsg = ADT<"addenda", Addenda.Msg>;
    ...
    addenda: immutable(
      await Addenda.init({
        ...
        async publishNewAddendum(value) {
          ...
          return (
            outcome || invalid(["Unable to add addenda due to a system error."])
          );
        }
      })
    )
...
export const component: Tab.Component<State, Msg> = {
  ...
  getModal(state) {
    return mapPageModalGlobalComponentMsg(
      Addenda.getModal(state.addenda),
      (value) => adt("addenda", value)
    );
  },
