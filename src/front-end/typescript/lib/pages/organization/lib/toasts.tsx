...
export const addedTeamMembers = {
  success: (emails: string[]) => ({
    ...
    body: (
      <div>
        <p>
          Successfully invited the following people to join this organization:
        </p>
        <ul>
          {emails.map((e, i) => (
            <li key={`organization-add-team-members-success-toast-${i}`}>
              {e}
            </li>
          ))}
        </ul>
  ...
  warning: (emails: string[]) => ({
    ...
    body: (
      <div>
        <p>
          The following people have not yet registered with the Digital
          Marketplace. However, they have been sent an email that invites them
          to create an account. Once they have registered, please try adding
          them to this organization again.
        </p>
        <ul>
          {emails.map((e, i) => (
            <li key={`organization-add-team-members-warning-toast-${i}`}>
              {e}
            </li>
          ))}
        </ul>
