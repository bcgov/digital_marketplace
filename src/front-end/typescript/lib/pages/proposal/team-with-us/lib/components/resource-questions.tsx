...
export const init: component_.base.Init<Params, State, Msg> = ({
  ...
  const responseInits = questions.map((question, i) => {
    const [responseState, responseCmds] = RichMarkdownEditor.init({
      errors: [],
      validate: (v) =>
        proposalValidation.validateTWUProposalResourceQuestionResponseResponse(
          v,
          question.wordLimit
        ),
      child: {
        value: find(responses, { order: question.order })?.response || "",
        id: `twu-proposal-team-question-response-${question.order}`,
...
export type Values = CreateTWUProposalResourceQuestionResponseBody[];
