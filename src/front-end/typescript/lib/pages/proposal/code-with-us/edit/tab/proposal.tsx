    ...
    case "startEditing":
      return [
        ...
        async (state) => {
          ...
          const proposalResult = await api.proposals.cwu.readOne(
            state.proposal.opportunity.id,
            state.proposal.id
          );
          const affiliationsResult = await api.affiliations.readMany();
          if (
            !api.isValid(proposalResult) ||
            !api.isValid(affiliationsResult)
          ) {
            return state;
          }
          state = state
    ...
    case "saveChanges":
      ...
      return [
        ...
        async (state, dispatch) => {
          ...
          const isSave =
            state.proposal.status === CWUProposalStatus.Draft ||
            state.proposal.status === CWUProposalStatus.Withdrawn;
          const errorToast = () => {
            dispatch(
              toast(
                adt(
    ...
    case "saveChangesAndSubmit":
      ...
      return [
        ...
        async (state, dispatch) => {
          ...
          const submitResult = await api.proposals.cwu.update(
            state.proposal.id,
            adt("submit", "")
          );
