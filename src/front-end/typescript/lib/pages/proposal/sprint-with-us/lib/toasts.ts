...
import {
  swuProposalStatusToPastTenseVerb,
  swuProposalStatusToPresentTenseVerb
...
export const statusChanged = {
  ...
  error: (s: SWUProposalStatus) => {
    return {
      ...
      body: `Sprint With Us proposal could not be ${swuProposalStatusToPastTenseVerb(
        s
...
type ScoreTest = "Team Questions" | "Code Challenge" | "Team Scenario";

export const scored = {
  success: (test: ScoreTest) => ({
    title: `${test} Scored`,
    body: `Sprint With Us proposal's ${test} ha${
      test === "Team Questions" ? "ve" : "s"
    } been scored.`
  }),
