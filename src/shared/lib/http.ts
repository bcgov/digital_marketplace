...
export function isValid<T>(
  v: ResponseValidation<T, unknown>
): v is ADT<"valid", T> {
  return v.tag === "valid";
}

export function isInvalid<T>(
  v: ResponseValidation<unknown, T>
): v is ADT<"invalid", T> {
  return v.tag === "invalid";
}

export function isUnhandled(
  v: ResponseValidation<unknown, unknown>
): v is ADT<"unhandled"> {
  return v.tag === "unhandled";
}

export function getValidValue<T>(
  v: ResponseValidation<T, unknown>,
  fallback: T
): T {
...
export function getInvalidValue<T>(
  v: ResponseValidation<unknown, T>,
  fallback: T
): T {
...
export function mapValid<A, B, C>(
  value: ResponseValidation<A, B>,
  fn: (b: A) => C
): ResponseValidation<C, B> {
  switch (value.tag) {
    case "valid":
      return valid<C>(fn(value.value));
    default:
      return value;
  }
}
...
export function mapInvalid<A, B, C>(
  value: ResponseValidation<A, B>,
  fn: (b: B) => C
): ResponseValidation<A, C> {
  switch (value.tag) {
    case "invalid":
      return invalid<C>(fn(value.value));
    default:
      return value;
  }
}
...
export type RequestFunction = <Data>(
...
export function prefixRequest(prefix: string): RequestFunction {
  const cleanSlashes = (v: string): string =>
    v.replace(/^\/*/, "/").replace(/\/*$/, "");
  return (method, path, data) => {
    return request(
      method,
      `${cleanSlashes(prefix)}${cleanSlashes(path)}`,
      data
    );
  };
}
