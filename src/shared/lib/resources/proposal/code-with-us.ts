...
export function compareCWUProposalStatuses(
  a: CWUProposalStatus,
  b: CWUProposalStatus
): Comparison {
  return compareNumbers(
    quantifyCWUProposalStatusForSort(a),
    quantifyCWUProposalStatusForSort(b)
  );
}

export function compareCWUProposalsForPublicSector(
  a: CWUProposalSlim,
  b: CWUProposalSlim
): Comparison {
...
export function getCWUProponentName(p: CWUProposal | CWUProposalSlim): string {
  switch (p.proponent.tag) {
    case "individual":
      return p.proponent.value.legalName;
    case "organization":
      return p.proponent.value.legalName;
  }
}
...
export function getCWUProponentEmail(p: CWUProposal | CWUProposalSlim): string {
  switch (p.proponent.tag) {
    case "individual":
      return p.proponent.value.email;
    case "organization":
      return p.proponent.value.contactEmail;
  }
}

export function getCWUProponentId(
  p: CWUProposal | CWUProposalSlim
): ADT<"individual", Id> | ADT<"organization", Id> {
  return adt(p.proponent.tag, p.proponent.value.id);
}

export function getCWUProponentTypeTitleCase(
  p: CWUProposal | CWUProposalSlim
): string {
  switch (p.proponent.tag) {
    case "individual":
...
export interface CreateIndividualProponentRequestBody
  extends Omit<CWUIndividualProponent, "id" | "phone" | "street2"> {
  phone: string | null;
  street2: string | null;
}
...
export function isValidStatusChange(
  from: CWUProposalStatus,
  to: CWUProposalStatus,
  userType: UserType,
  ...
  switch (from) {
    ...
    case CWUProposalStatus.NotAwarded:
      return (
        [CWUProposalStatus.Awarded, CWUProposalStatus.Disqualified].includes(
          to
        ) &&
        userType !== UserType.Vendor &&
        hasProposalDeadlinePassed
...
export function canCWUProposalBeAwarded(
  p: Pick<CWUProposal, "status">
): boolean {
  switch (p.status) {
    case CWUProposalStatus.NotAwarded:
    case CWUProposalStatus.Evaluated:
...
export function isCWUProposalStatusVisibleToGovernment(
  s: CWUProposalStatus,
