...
export function validateCWUProposalStatus(
  raw: string,
  isOneOf: CWUProposalStatus[]
): Validation<CWUProposalStatus> {
  const parsed = parseCWUProposalStatus(raw);
  if (!parsed) {
    return invalid([`"${raw}" is not a valid CodeWithUs proposal status.`]);
  }
