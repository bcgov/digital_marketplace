...
Cypress.Commands.add("sqlFixture", (sqlFilename) => {
  cy.exec(
    `docker exec dm_db psql -v "ON_ERROR_STOP=1" -U digmkt digmkt -f /workspace/cypress/fixtures/${sqlFilename}`
  ); //need /workspace/ because that's the file structure in the db container
});
...
Cypress.Commands.add("scoreCodeChallenge", (proponent, score) => {
  ...
  cy.get("a").contains("Screen In").click();
  cy.get("a").contains("Back to Opportunity").click();
  cy.get('div[class*="toast-header"]')
    .children("svg")
    .click({ multiple: true, force: true }); // clear toasts (they pile up and cover the whole right side of the window)
});
...
Cypress.Commands.add("scoreTeamScenario", (proponent, score) => {
  ...
  cy.get('div[class*="toast-header"]')
    .children("svg")
    .click({ multiple: true, force: true }); //clear toasts
});
...
Cypress.Commands.add("evaluateStage", (evaluationStage) => {
  ...
  cy.get('div[class*="modal-footer"]')
    .children()
    .contains(`Complete ${evaluationStage}`)
    .click();
});

Cypress.Commands.add(
  "checkEvalTableRow",
  (proponent, evalStage, tqScore, ccScore, tsScore, priceScore, total) => {
    //check table headers
