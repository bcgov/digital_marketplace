...
describe("As a user authenticated via GitHub,", function () {
  ...
  it("edit CWU proposal", function () {
    ...
    cy.get('div[class*="modal-footer"]')
      ...
      .click();
    cy.contains(
      "Your changes to your Code With Us proposal have been submitted."
    ).should("be.visible");
    ...
    cy.get("#cwu-proposal-individual-legalName").should(
      "have.value",
      "new legal name"
    );
    cy.get("#cwu-proposal-individual-email").should(
      "have.value",
      "new@gmail.com"
    );
    cy.get("#cwu-proposal-individual-phone").should(
      "have.value",
      "222-222-2222"
    );
    cy.get("#cwu-proposal-individual-street1").should(
      "have.value",
      "new address1"
    );
    cy.get("#cwu-proposal-individual-street2").should(
      "have.value",
      "new address2"
    );
    cy.get("#cwu-proposal-individual-city").should("have.value", "new city");
    cy.get("#cwu-proposal-individual-region").should(
      "have.value",
      "new province"
    );
    cy.get("#cwu-proposal-individual-mailCode").should(
      "have.value",
      "new postal code"
    );
    cy.get("#cwu-proposal-individual-country").should(
      "have.value",
      "new country"
    );
    cy.get("a").contains("Next").click();
    ...
    cy.get("#cwu-proposal-additional-comments").should(
      "have.value",
  ...
  it("delete CWU proposal", function () {
    ...
    cy.get('div[class*="modal-footer"]')
      .children()
      .contains("Withdraw Proposal")
      .click();

    cy.contains("Code With Us proposal has been withdrawn.").should(
      "be.visible"
    );
