...
describe("As a user authenticated via IDIR", function () {
  ...
  it("create, publish, and read new CWU opportunity", function () {
    ...
    cy.get("#cwu-opportunity-acceptance-criteria").type(
      "Some acceptance criteria"
    );
    cy.get("#cwu-opportunity-evaluation-criteria").type(
    ...
    cy.contains("Code With Us opportunity has been published.").should(
      "be.visible"
    );
    ...
    cy.get("#cwu-opportunity-remote-desc").should(
      "have.value",
      "Remote description text"
    );
    cy.get("#cwu-opportunity-location").should("have.value", "Vancouver");
    cy.get("#cwu-opportunity-reward").should("have.value", "5000");
    cy.get("#cwu-opportunity-skills")
      .contains("Agile")
      .should("have.text", "Agile");
    cy.get("a").contains("Next").click();

    // 2. Description tab
    cy.get("#cwu-opportunity-description").should(
      "have.value",
      "Opp description"
    );
    cy.get("a").contains("Next").click();
    ...
    // 3. Details tab
    cy.get("#cwu-opportunity-proposal-deadline").should(
      "have.value",
      "2030-01-15"
    );
    cy.get("#cwu-opportunity-assignment-date").should(
      "have.value",
      "2030-01-31"
    );
    cy.get("#cwu-opportunity-start-date").should("have.value", "2030-02-15");
    cy.get("#cwu-opportunity-completion-date").should(
      "have.value",
      "2030-02-28"
    );
    cy.get("#cwu-opportunity-submission-info").should(
      "have.value",
      "github repo"
    );
    cy.get("#cwu-opportunity-acceptance-criteria").should(
      "have.value",
      "Some acceptance criteria"
    );
    cy.get("#cwu-opportunity-evaluation-criteria").should(
      "have.value",
      "Some evaluation criteria"
    );
    cy.get("a").contains("Next").click();
