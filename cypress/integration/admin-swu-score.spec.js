...
describe("As an administrator authenticated via IDIR", function () {
  ...
  it("advances proponents 1 and 2 through all evaluations and awards opportunity to proponent 1", function () {
    ...
    // Award opportunity
    cy.get('div[class="table-show-on-hover"]')
      .children("a")
      .contains("Award")
      .click({ force: true }); //forcing because it requires hover
    cy.get('div[class*="modal-footer"]')
      .children()
      .contains("Award Opportunity")
      .click();
    cy.contains("Sprint With Us opportunity has been awarded.").should(
      "be.visible"
    );
