...
describe("As a user authenticated via IDIR", function () {
  ...
  it("create and submit SWU for review, read opportunity", function () {
    ...
    cy.get('a[href="/opportunities/sprint-with-us/create"]').should(
      "be.visible"
    );
    ...
    cy.get("#swu-opportunity-mandatory-skills").type(
      "Front-End Development{enter}"
    );
    cy.get("#swu-opportunity-optional-skills").type(
      "Adobe Creative Cloud{enter}"
    );
    ...
    cy.get('div[class="pt-2 pb-4 mb-4"]')
      .eq(0)
      .contains("Delivery Management")
      .click();
    cy.contains("P/T").click();
    cy.get('div[class="pt-2 pb-4 mb-4"]')
      .eq(0)
      .contains("User Research")
      .click();
    ...
    cy.get('div[class*="modal-footer"]')
      ...
      .click();
    cy.contains("Sprint With Us opportunity has been submitted.").should(
      "be.visible"
    );
    ...
    cy.get('h4[class="mb-0"]').should(
      "contain.text",
      "Code Challenge Participants"
    );
    ...
    cy.get("#swu-opportunity-remote-desc").should(
      "have.value",
      "SWU cy remote desc"
    );
    cy.get("#swu-opportunity-location").should("have.value", "Sechelt");
    cy.get("#swu-opportunity-proposal-deadline").should(
      "have.value",
      "2030-01-01"
    );
    cy.get("#swu-opportunity-assignment-date").should(
      "have.value",
      "2030-01-02"
    );
    ...
    cy.get("#swu-opportunity-mandatory-skills")
      .contains("Back-End Development")
      .should("have.text", "Back-End Development");
    cy.get("#swu-opportunity-mandatory-skills")
      .contains("Front-End Development")
      .should("have.text", "Front-End Development");
    cy.get("#swu-opportunity-optional-skills")
      .contains("Adobe Creative Cloud")
      .should("have.text", "Adobe Creative Cloud");
    cy.get("a").contains("Next").click();

    // 2. Description tab
    cy.get("#swu-opportunity-description").should(
      "have.value",
      "SWU cy description"
    );
    cy.get("a").contains("Next").click();
    ...
    cy.get('div[class="pt-2 pb-4 mb-4"]')
      .eq(0)
      .contains("Delivery Management")
      .should("include.text", "Delivery Management");
    cy.contains("P/T").should("have.text", "P/T");
    cy.get('div[class="pt-2 pb-4 mb-4"]')
      .eq(0)
      .contains("User Research")
      .should("include.text", "User Research");
    ...
    cy.get('div[class="pt-2 pb-4 mb-4"]')
      .eq(1)
      .contains("Frontend Development")
      .should("include.text", "Frontend Development");
    cy.get('div[class="pt-2 pb-4 mb-4"]')
      .eq(1)
      .contains("Technical Architecture")
      .should("include.text", "Technical Architecture");
    ...
    cy.get('div[class="pt-2 pb-4 "]')
      .contains("Security Engineering")
      .should("include.text", "Security Engineering");
    cy.get('div[class="pt-2 pb-4 "]')
      .contains("Backend Development")
      .should("include.text", "Backend Development");

    cy.get("a").contains("Next").click();

    // 4. Team questions
    cy.get("[id*=team-questions-question]").should(
      "have.value",
      "SWU cy question"
    ); // would need to change this selector if entering multiple questions
    cy.get('[id*="team-questions-response-guidelines"]').should(
      "have.value",
      "SWU cy response"
    );
