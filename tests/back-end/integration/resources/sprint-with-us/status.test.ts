...
describe("SWU Opportunity and Proposal Resources", () => {
  ...
  it("should properly update opportunity and proposal statuses when an opportunity is closed and awarded", async () => {
    ...
    // Screen in proposals to team scenario
    ...
    const updatedProposalTS2 = getValidValue(
      await updateSWUProposalStatus(
        connection,
        swuProposal2.id,
        SWUProposalStatus.UnderReviewTeamScenario,
    ...
    expect(updatedProposalTS2?.status).toEqual(
      SWUProposalStatus.UnderReviewTeamScenario
    );
    ...
    const updatedProposalPostTS2 = getValidValue(
      await updateSWUProposalScenarioAndPriceScores(
        connection,
        updatedProposalTQ2.id,
        70,
    ...
    expect(updatedProposalPostTS2?.status).toEqual(
      SWUProposalStatus.EvaluatedTeamScenario
    );
