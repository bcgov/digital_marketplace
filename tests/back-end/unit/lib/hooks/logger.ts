...
function makeStubRequest(
  onLog: (kind: LogFunctionKind, msg: string, data?: object) => void
): Request<null, Session> {
  const makeLogFn = (kind: LogFunctionKind) => (msg: string, data?: object) =>
    onLog(kind, msg, data);
  return {
    headers: {},
    params: {},
  ...
  describe("before", function () {
    describe("when provided a request", function () {
      it("calls any function on the logger", async function () {
        let called = false;
        const request = makeStubRequest((_) => {
          called = true;
        });
      ...
      it("returns the current epoch time", async function () {
        ...
        assert(
          ...
          "the difference between the current time and the timestamp is reasonably small"
        );
      });
    });
  });
